...
def handle_404(error):
    # Browsers request /favicon.ico automatically. Never turn a missing favicon into a 500.
...
def handle_413(error):
    return render_template(
        ...
        error_message="Le fichier envoyé dépasse la taille autorisée.",
        home_url="/"
    ), 413

@app.errorhandler(500)
def handle_500(error):
    # Do not let the error page itself generate another exception.
    try:
        return render_template(
            "error.html",
            error_code=500,
        ...
        return (
            ...
            "<p>Veuillez réessayer ultérieurement.</p></html>"
        ), 500
